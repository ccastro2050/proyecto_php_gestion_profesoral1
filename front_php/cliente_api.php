 ...
 *
 * para usar aquí la clase `Programa` de la API. Funcionaría. Y estaría mal: los
...
define('URL_API', getenv('URL_API') ?: 'http://localhost:8113');
 ...
 * (proyecto_paradigmas_gestion_profesoral1) envuelve todo eso dentro de
...
// ======================================================================
// LAS CINCO OPERACIONES SOBRE `programa`
//
...
function listar_gestion(int $limite = 1000): array
{
    $r = llamar_api('GET', "/api/programa?limite=$limite");
...
function obtener_gestion(string $clave): array
{
    $r = llamar_api('GET', '/api/programa/' . rawurlencode($clave));
...
/** RF3 — Crear. */
function crear_gestion(array $datos): array
{
    return resultado_de(llamar_api('POST', '/api/programa', $datos));
}
 ...
 */
function reemplazar_gestion(string $clave, array $datos): array
{
    return resultado_de(llamar_api('PUT', '/api/programa/' . rawurlencode($clave), $datos));
}
 ...
 */
function actualizar_gestion(string $clave, array $datos): array
{
    return resultado_de(llamar_api('PATCH', '/api/programa/' . rawurlencode($clave), $datos));
}
 ...
 */
function eliminar_gestion(string $clave): array
{
    return resultado_de(llamar_api('DELETE', '/api/programa/' . rawurlencode($clave)));
}
