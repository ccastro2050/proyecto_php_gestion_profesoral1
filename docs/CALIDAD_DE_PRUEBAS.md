...
public function testCrearPrograma(): void
{
    $creado = $this->servicio->crearPrograma($datos);
    // (sin assert: si crearPrograma guarda mal, esta prueba PASA)
}
```
...
```php
public function testCrearProgramaGuardaElNombre(): void
{
    $creado = $this->servicio->crearPrograma($datos);
