...
tiene 2: `./api_gestion`, `./front_php`.

Este es el de `api-gestion`, sin los comentarios para verlo de un vistazo:
...
COPY . .
EXPOSE 8113
CMD ["php", "-S", "0.0.0.0:8113", "index.php"]
```
...
| `EXPOSE 8113` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
| `CMD ["php", "-S", "0.0.0.0:8113", "index.ph…` | **El comando que se ejecuta al encender** el contenedor | Si ese proceso termina, el contenedor se apaga |
...
|---|---|---|
| `api-gestion` | **Sí**, en `./api_gestion` | Es código **suyo**: nadie más lo tiene, hay que armarlo |
...
- En la v1: `proyecto_php1-mariadb-1` es un contenedor creado desde la imagen
  `mariadb:11`, con el puerto interno 3306 **publicado** en el 13331 de su PC
  (`"13331:3306"` en el compose).
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTPHP["SERVIDOR WEB (front)<br/>contenedor front-php<br/>hostname: front-php · escucha en 8112"]
            APIGESTION["SERVIDOR DE APLICACIONES<br/>contenedor api-gestion<br/>hostname: api-gestion · escucha en 8113"]
    ...
    NAV -->|"localhost:8113"| APIGESTION
    NAV -->|"localhost:8112"| FRONTPHP
    NAV -->|"localhost:8106"| PHPMYADMIN
    APIGESTION -->|"mariadb:3306 (DNS de Docker)"| MARIADB
    FRONTPHP -->|"http://api-gestion:8113<br/>al controlador, por el NOMBRE"| APIGESTION
    PHPMYADMIN -->|"mariadb:3306 (DNS de Docker)"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13331"| MARIADB
```
...
services:                          # el mapa de TODOS los contenedores del sistema
  ...
  mariadb:                         # ← este nombre es también su HOSTNAME interno
    ...
    ports:
      - "13331:3306"               # "puerto en su PC : puerto interno del contenedor"
  ...
  api-registros:
    build: ./api_gestion          # esta imagen SE CONSTRUYE con el Dockerfile de esa carpeta
    volumes:
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8113`, `localhost:8101`, `localhost:13331`); entre
...
docker network create proyecto_php_gestion_profesoral1_default

# 2. mariadb
docker run -d --name mariadb --network proyecto_php_gestion_profesoral1_default --restart unless-stopped `
  -e "MARIADB_ROOT_PASSWORD=paradigmas123" `
  -e "MARIADB_DATABASE=gestion_php_local" `
  -e "MARIADB_USER=gestion" `
  -e "MARIADB_PASSWORD=paradigmas123" `
  -v mariadbdata:/var/lib/mysql `
  -v "${PWD}/db/init.sql:/docker-entrypoint-initdb.d/init.sql:ro" `
  -p 13331:3306 mariadb:11

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. Construir la imagen de api-gestion y encenderla
docker build -t api-gestion ./api_gestion
docker run -d --name api-gestion --network proyecto_php_gestion_profesoral1_default --restart unless-stopped `
  -e "DB_DSN=mysql:host=mariadb;port=3306;dbname=gestion_php_local" `
  -e "DB_USUARIO=gestion" `
  -e "DB_CLAVE=paradigmas123" `
  -v "${PWD}/api_gestion:/app" `
  -p 8113:8113 api-gestion

# 5. Construir la imagen de front-php y encenderla
docker build -t front-php ./front_php
docker run -d --name front-php --network proyecto_php_gestion_profesoral1_default --restart unless-stopped `
  -e "URL_API=http://api-gestion:8113" `
  -v "${PWD}/front_php:/app" `
  -p 8112:8112 front-php

# 6. phpmyadmin
docker run -d --name phpmyadmin --network proyecto_php_gestion_profesoral1_default --restart unless-stopped `
  -e "PMA_HOST=mariadb" `
  -e "PMA_USER=gestion" `
  -e "PMA_PASSWORD=paradigmas123" `
  -p 8106:80 phpmyadmin:latest
...
**`api-gestion` es un nombre que solo existe dentro de la red de Docker.**
Su navegador corre en Windows, fuera de esa red, y no sabe quién es.

| Desde dónde | Qué dirección sirve | Por qué |
|---|---|---|
| **Su navegador** | `http://localhost:8113` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-gestion:8113` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8113:8113`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8113`, el front
...
```powershell
docker network inspect proyecto_php_gestion_profesoral1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_php_gestion_profesoral1` produce
la red `proyecto_php_gestion_profesoral1_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_php_gestion_profesoral1-front-php-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_php_gestion_profesoral1_default has active endpoints (name:"proyecto_php_gestion_profesoral1-front-php-1" …)
```
...
6. En este repositorio: el `docker run` de la v1 en el
   ...
   [modelo de datos de la v1](spec_kit/versiones/v1_programa/5_data_model.md).
