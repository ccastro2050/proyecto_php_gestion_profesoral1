...
```
            EL CLIENTE (navegador, Postman, curl)
                 │
                 │  ① GET /api/programa/PR001
                 ▼
┌─────────────────────────────────────────────────────┐
│ CAPA 1 — CONTROLADOR (HTTP)                         │
│ controladores/ControladorPrograma.php               │
...
│ servicios/ServicioPrograma.php                      │
...
└────────────────┬────────────────────────────────────┘
                 ...
                 │     — a través de la INTERFAZ IRepositorioPrograma
                 ▼
┌─────────────────────────────────────────────────────┐
│ CAPA 3 — REPOSITORIO (datos)                        │
│ repositorios/RepositorioProgramaMariaDB.php         │
│ El SQL con PDO: traduce filas ↔ objetos Programa.   │
│ NO conoce HTTP. NO decide negocio.                  │
└────────────────┬────────────────────────────────────┘
                 │  ④ SELECT … FROM programa WHERE codigo = :codigo
                 ▼
          ┌───────────────┐
          │ BASE DE DATOS │  MariaDB — gestion_php_local
          └───────┬───────┘
                  │
   y la respuesta hace el viaje DE VUELTA:
   fila → objeto Programa (repositorio) → objeto (servicio)
...
|---|---|---|---|
| **Controlador** | HTTP: validar el body, códigos de estado, JSON | SQL y reglas de negocio | `controladores/ControladorPrograma.php` |
| **Servicio** | Las reglas del negocio (¿existe? ¿se puede?) | Saber de HTTP o del motor de BD | `servicios/ServicioPrograma.php` |
| **Repositorio** | El SQL y el mapeo fila ↔ objeto | Saber de HTTP o decidir negocio | `repositorios/RepositorioProgramaMariaDB.php` |
...
**El mismo viaje cuando algo sale mal** — `GET /api/programa/PR999`:

1. El **repositorio** no encuentra la fila y devuelve `null` — un HECHO,
   sin opinión.
2. El **servicio** decide qué significa ese hecho: "ese programa no
...
```
CAPA 1: FRONT (:8112)   → solo pinta y llama la API; nunca toca la BD
CAPA 2: API   (:8113)   → solo JSON
...
Pregunta legítima: la carpeta de modelos (`modelos/Programa.php`) existe en el
...
- El **modelo es LO QUE VIAJA entre estaciones**: el repositorio arma un
  `Programa` desde la fila, el servicio lo razona, el controlador lo
...
flowchart LR
    ...
    M["MODELO Programa<br/>el paquete que viaja"]
    C -.->|"lo conoce"| M
    S -.->|"lo conoce"| M
    R -.->|"lo conoce"| M
...
`ciudad` no lo atraparía nadie hasta producción. Con modelo, lo atrapa el
lenguaje. En PHP el modelo es la clase con propiedades tipadas — el `ciudad` que
...
if ($ruta === '/api/programa' && $metodo === 'GET') {
...
(`RepositorioProgramaPostgreSQL implements IRepositorioPrograma`) y ajustar el
...
class RepositorioProgramaPostgreSQL implements IRepositorioPrograma { /* … */ }

// ...y el ensamblador (ÚNICO archivo tocado) elegirá el motor:
$repositorio = $motor === 'postgres'
    ? new RepositorioProgramaPostgreSQL($dsn, $usuario, $clave)
    : new RepositorioProgramaMariaDB($dsn, $usuario, $clave);
```
...
`RepositorioProgramaMariaDB` son indistinguibles para el servicio — por eso la
...
{
    public function obtenerPorCodigo(string $codigo): ?Programa
    {
...
**En la v1:** `IRepositorioPrograma` tiene exactamente los 5 métodos del CRUD
de programa — no un `IRepositorioUniversal` con 40 métodos. Cuando la v2
agregue persona, tendrá SU interfaz.

```php
// ✅ La interfaz de la v1: SOLO los 5 métodos del CRUD de programa
...
**En la v1:** `ServicioPrograma` recibe **la interfaz** por constructor
(`private readonly IRepositorioPrograma $repositorio`); solo
...
class ServicioPrograma implements IServicioPrograma
{
    public function __construct(
        private readonly IRepositorioPrograma $repositorio,  // ← interfaz, no clase
    ) {
    }
}

// El ÚNICO lugar que conoce la clase concreta (v3 lo convertirá en fábrica):
function crearServicioPrograma(): IServicioPrograma
{
    $repositorio = new RepositorioProgramaMariaDB(
        getenv('DB_DSN'), getenv('DB_USUARIO'), getenv('DB_CLAVE')
    );
    return new ServicioPrograma($repositorio);
}
```
...
match (true) {
    $ruta === '/api/programa' && $metodo === 'GET' => $controlador->listar(),
    // …
};
```
...
```php
// El contrato (repositorios/IRepositorioPrograma.php):
public function obtenerPorCodigo(string $codigo): ?Programa;

// ServicioPrograma lo usa SIN saber si detrás hay MariaDB o un array
...
{
    public function __construct(
        private readonly IRepositorioPrograma $repositorio,  // ← llega armado
    ) {
    }
}
```
...
function crearRepositorioPrograma(string $motor): IRepositorioPrograma
{
    return $motor === 'postgres'
        ? new RepositorioProgramaPostgreSQL($dsnPg, $usuario, $clave)
        : new RepositorioProgramaMariaDB($dsnMaria, $usuario, $clave);
}
...
(`RepositorioProgramaMariaDB`, `RepositorioFalsoEnMemoria` — y los motores
...
7. En este repositorio: el [plan de la v1](spec_kit/versiones/v1_programa/3_plan.md)
