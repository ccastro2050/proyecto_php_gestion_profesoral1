...
|---|---|---|
| **Lista** — llaves 0,1,2… | `List<T>`, `T[]` | La lista de programas que devuelve `obtenerTodos()` |
| **Diccionario** — llaves de texto | `Dictionary<string,T>`, `Map` | `$datos` en el `UPDATE` dinámico: columna → valor |
| **Registro** — llaves fijas y conocidas | una `struct`, una clase | La fila que devuelve PDO: `['id' => …, 'nombre' => …]` |
...
$programas[10101] = ['id' => 10101, 'nombre' => 'Primera'];
$programas[10102] = ['id' => 10102, 'nombre' => 'Segunda'];

// ---------- READ: una sola, y todas ----------
echo $programas[10101]['nombre'], "
";        // Primera
echo $programas[99999]['nombre'] ?? 'no existe', "
";   // no existe

foreach ($programas as $llave => $ficha) {
    echo $llave, ' -> ', $ficha['nombre'], "
";
}

// ---------- UPDATE: cambiar un campo ----------
$programas[10101]['nombre'] = 'Corregida';
echo $programas[10101]['nombre'], "
";        // Corregida

// ---------- DELETE: quitar la ficha ----------
unset($programas[10101]);
...
- **El UPDATE de arriba SÍ funciona**, y es importante entender por qué:
  `$programas[…]['nombre'] = 'Corregida'` escribe **directo en el
  almacén**. Si en cambio hubiera hecho `$ficha = $programas[…];` y luego
...
**`api_gestion/pruebas/prueba_capas.php`**. Ahí adentro hay una clase que cumple
...
```php
public function obtenerPorClave($clave): ?Programa
{
...
Ejemplo real, del controlador (`api_gestion/controladores/ControladorPrograma.php`):
...
<?php else: ?>                    <!-- no hay: "todavía no hay programas" -->
```
...
objetos del modelo (`api_gestion/repositorios/RepositorioProgramaMariaDB.php`):
...
Con `$datos = ['nombre' => 'X', 'facultad' => 9]` sale:

```sql
UPDATE programa SET nombre = :nombre, facultad = :facultad WHERE id = :clave_de_la_fila
```
...
(`api_gestion/index.php`):

```php
$metodo = $_SERVER['REQUEST_METHOD'];                          // "GET", "POST", "PUT"…
$ruta   = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);    // "/api/programa"
```
...
Si se comparara `$_SERVER['REQUEST_URI'] === '/api/programa'`, la petición
`/api/programa?limite=5` **no coincidiría con ninguna ruta** y daría 404.
...
|---|---|---|
| Inyección de SQL | Los valores **nunca** se pegan al SQL: van como parámetros de PDO | `api_gestion/repositorios/RepositorioProgramaMariaDB.php` |
...
1. **PDO devuelve arreglos.** `fetchAll(PDO::FETCH_ASSOC)` entrega cada fila
   como `['id' => …, 'nombre' => …]`, con **todo como texto**
...
1. Abra `api_gestion/pruebas/prueba_capas.php`, busque en el repositorio falso
...
| | |
|---|---|
| [3_plan.md](spec_kit/versiones/v1_programa/3_plan.md) | Las capas de esta versión y por qué están cortadas ahí |
| [6_contracts.md](spec_kit/versiones/v1_programa/6_contracts.md) | El sobre del JSON: qué array recibe el front en cada respuesta |
