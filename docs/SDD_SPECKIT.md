...
> 5. **El código pasa a ser un subprograma:** si toda la lógica vive en
...
specify init mi_v1_programa --integration copilot
specify version
```
...
```text
La spec dice:
    RF6 — Eliminar programa
    DELETE /api/programa/{codigo}. Devuelve filasEliminadas;
    inexistente -> 404.

clarify pregunta:
    El borrado de programa, ¿es físico o lógico?
...
clarify NO crea un archivo: escribe dentro de 2_spec.md
    ## Clarificaciones
    C5 — DELETE, ¿físico o lógico? -> Físico: la tabel programa no
    tiene columna de estado. El borrado lógico llega con la anulación
    de registros, en una versión posterior.
```

**A mano, en este curso:** eso es exactamente la
[la sección de Clarificaciones de 2_spec.md](spec_kit/versiones/v1_programa/2_spec.md),
...
Convergencia — v1
    ...
    Criterio 2  OK    GET /api/programa devuelve las 8 filas semilla
...
[8_tasks.md](spec_kit/versiones/v1_programa/8_tasks.md) —el
cierre— corriendo el smoke test del
[7_quickstart.md](spec_kit/versiones/v1_programa/7_quickstart.md).
...
4. **Es evaluable y comparable.** Los `.md` se leen y se califican, y los
   cursos comparten la misma estructura sobre la misma `gestion_php_local` con
   stacks distintos.
...
- **La versión en curso:** [spec_kit/versiones/v1_programa/](spec_kit/versiones/v1_programa/2_spec.md)
...
`PATCH /api/programa/{codigo}` con body parcial (campos opcionales):
...
```markdown
### RF5 — Actualizar parcialmente (PATCH + body parcial)
PATCH /api/programa/{codigo} con campos opcionales: solo se
...
| Tabla | PK | Semilla |
|---|---|---|
| programa | codigo | 8 filas (PR001 "Laptop…", stock 17, …) |

El stock lo mueve el TRIGGER al registror: la API tiene PROHIBIDO
...
```bash
curl http://localhost:8113/api/programa        # total: 8
curl -i http://localhost:8113/api/programa/PR999   # → 404
```
...
- [ ] modelos/Programa.php (la entidad: 4 propiedades)
- [ ] peticiones/ProgramaCrear.php (validación: todo obligatorio)
...
2. **Medible o no es criterio.** Un criterio dice un número, un código de
   ...
   200 con las 191 filas semilla" sí.
...
```markdown
### RF6 — Eliminar programa
DELETE /api/programa/{codigo} elimina el programa.
[NECESITA ACLARACIÓN: ¿borrado físico, o lógico como la anulación
de registros? Afecta al criterio 5 y al contrato del DELETE.]
```
...
| "Debe ser rápido" | "El listado responde en menos de 1 s con las 191 filas semilla" |
...
|---|---|---|---|---|
| RF1 — listar | §2 `GET /api/programa` | Fase 5 · controlador | §2 comando 2 | 1 y 2 |
...
1. La carpeta de la versión es `vN_tema`, en minúsculas y con guion bajo
   (`v1_programa`). El tema dice qué agrega la versión.
...
7. En este repositorio: la [constitución](spec_kit/1_constitution.md), el
   ...
   [spec kit de la v1](spec_kit/versiones/v1_programa/2_spec.md).
