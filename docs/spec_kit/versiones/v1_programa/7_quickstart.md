# Quickstart — v1: `programa` (PHP + MariaDB)
...
> falta: `curl -s -X POST $API/api/programa -H "Content-Type: application/json" -d '…'`
> funciona tal cual.
...
pedir GET /
#    → mensaje: API de Gestión profesoral funcionando · version: v1 · tabla: programa

# 2. El sistema arranca CON DATOS: el init.sql sembró la tabla
(pedir GET /api/programa).total
#    → 191
#      El 204 sigue siendo la respuesta al listado VACÍO —y el front lo trata
#      como «todavía no hay»— pero no se llega ahí arrancando.

# 3. Crear y listar
pedir POST /api/programa '{"id": 9001, "nombre": "Ingenieria de Sistemas", "tipo": "Programa Academico", "nivel": "Pregrado", "fecha_creacion": "2005-01-15", "numero_cohortes": "40", "cant_graduados": "1250", "fecha_actualizacion": "2026-01-30", "ciudad": "Medellin", "facultad": 104, "fecha_cierre": null}'
#    → estado 200 · "Programa creado exitosamente."
(pedir GET /api/programa).total
#    → 192

# 4. El ciclo de los cinco verbos
pedir PUT /api/programa/9001 '{"nombre": "Ingenieria de Sistemas", "tipo": "Programa Academico", "nivel": "Pregrado", "fecha_creacion": "2005-01-15", "numero_cohortes": "40", "cant_graduados": "1250", "fecha_actualizacion": "2026-01-30", "ciudad": "Medellin", "facultad": 104, "fecha_cierre": null}'
#    → estado 200 · filasAfectadas 1

pedir PATCH /api/programa/9001 '{"facultad": 104}'
#    → estado 200 · filasAfectadas 1

pedir GET /api/programa/9001
#    → la ficha, con el valor nuevo

# 4b. La pareja que enseña la diferencia: MISMO cuerpo, dos verbos
pedir PUT /api/programa/9001 '{"tipo": "Programa Academico", "nivel": "Pregrado", "fecha_creacion": "2005-01-15", "numero_cohortes": "40", "cant_graduados": "1250", "fecha_actualizacion": "2026-01-30", "ciudad": "Medellin", "facultad": 104, "fecha_cierre": null}'
#    → HTTP 422: al PUT le falta 'nombre', y reemplazar exige todo

pedir PATCH /api/programa/9001 '{"tipo": "Programa Academico", "nivel": "Pregrado", "fecha_creacion": "2005-01-15", "numero_cohortes": "40", "cant_graduados": "1250", "fecha_actualizacion": "2026-01-30", "ciudad": "Medellin", "facultad": 104, "fecha_cierre": null}'
#    → estado 200: al PATCH le basta con lo enviado

# 5. El borrado es LÓGICO, y se comprueba
pedir DELETE /api/programa/9001
#    → estado 200 · filasAfectadas 1
(pedir GET /api/programa).total
#    → 191 otra vez
pedir DELETE /api/programa/9001
...
docker compose exec mariadb `
  mariadb -ugestion -pparadigmas123 gestion_php_local `
  -e "SELECT id, activo FROM programa WHERE id = 9001"
#    → 9001 | 0   ← sigue ahí, con activo en cero

# 6. La validación es la frontera: nada de esto llega a la base
pedir POST /api/programa '{"tipo": "Programa Academico", "nivel": "Pregrado", "fecha_creacion": "2005-01-15", "numero_cohortes": "40", "cant_graduados": "1250", "fecha_actualizacion": "2026-01-30", "ciudad": "Medellin", "facultad": 104, "fecha_cierre": null}'
#    → HTTP 422, y los errores VIENEN EN ESPAÑOL y nombran el campo:
#      "El campo id es obligatorio…", "El campo nombre es obligatorio."
...
Abrir **http://localhost:8112** y:

1. entrar a **Programas**: se ven las 191 filas;
...
| `Connection refused` en el 8113 | La API no arrancó. `docker compose logs api-gestion` |
