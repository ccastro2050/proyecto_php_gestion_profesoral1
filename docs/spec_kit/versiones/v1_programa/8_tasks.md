# Tareas — v1: `programa` (PHP + MariaDB)
...
- [ ] `docker-compose.yml` con los cuatro servicios: `mariadb`, `api-gestion`,
...
docker compose exec mariadb mariadb -ugestion -pparadigmas123 gestion_php_local `
  -e "SELECT COUNT(*) FROM programa"
#  → 191
```

## Fase 1 — El modelo

- [ ] `modelos/Programa.php`: propiedades privadas, getters, setters para todo
      **menos la llave**, y `toArray()`.
- [ ] **Sin** `activo`: no es un campo de la ficha (5_data_model §4).

## Fase 2 — Las interfaces

- [ ] `repositorios/IRepositorioPrograma.php` y `servicios/IServicioPrograma.php`.
...
- [ ] `repositorios/RepositorioProgramaMariaDB.php` con PDO y prepared statements.
...
```powershell
docker compose exec api-gestion php -r "require 'servicios/ensamblador.php'; print_r(count(crearServicioPrograma()->listar(1000)));"
#  → 191
```

## Fase 4 — El servicio

- [ ] `servicios/ServicioPrograma.php`, que lanza `InvalidArgumentException` y
      `NoEncontradoExcepcion` — **nunca códigos HTTP**.
- [ ] `servicios/ensamblador.php`: el único `new` de clases concretas.

## Fase 5 — El controlador

- [ ] `controladores/ControladorPrograma.php` con la validación del cuerpo, la
