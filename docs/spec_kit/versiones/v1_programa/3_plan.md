# Plan técnico — v1: `programa` (PHP + MariaDB)
...
├── repositorios/
│   ├── IRepositorioPrograma.php
│   └── RepositorioProgramaMariaDB.php
│                           El SQL, en prepared statements. Único que conoce PDO.
├── modelos/Programa.php     La fila como objeto: privadas + getters + setters
...
flowchart TD
    ...
    R --> C["ControladorPrograma<br/>¿el cuerpo tiene la forma? si no → 422"]
    C -->|"interfaz IServicioPrograma"| S["ServicioPrograma<br/>las reglas"]
    S -->|"interfaz IRepositorioPrograma"| Rep["RepositorioProgramaMariaDB<br/>el SQL parametrizado"]
    Rep --> BD[("MariaDB")]
...
`IServicioPrograma` e `IRepositorioPrograma` son interfaces nativas de PHP. **No las
...
Con `php -S 0.0.0.0:8112 index.php`, PHP ejecuta el router
