# Guía de IA — v1: `programa` (PHP + MariaDB)
...
```powershell
# Las carpetas
mkdir docs\spec_kit\versiones\v1_programa, db, front_php\publico, api_gestion, api_gestion\controladores, api_gestion\excepciones, api_gestion\modelos, api_gestion\pruebas, api_gestion\repositorios, api_gestion\servicios, front_php, front_php\vistas, pruebas_humo

# Los archivos, vacíos: la IA los llena, uno por uno, y usted los revisa
New-Item .gitattributes, .gitignore, api_gestion\Dockerfile, api_gestion\controladores\ControladorPrograma.php, api_gestion\excepciones\NoEncontradoExcepcion.php, api_gestion\index.php, api_gestion\modelos\Programa.php, api_gestion\pruebas\prueba_capas.php, api_gestion\repositorios\IRepositorioPrograma.php, api_gestion\repositorios\RepositorioProgramaMariaDB.php, api_gestion\servicios\IServicioPrograma.php, api_gestion\servicios\ServicioPrograma.php, api_gestion\servicios\ensamblador.php, docker-compose.yml, front_php\Dockerfile, front_php\cliente_api.php, front_php\index.php, front_php\vistas\formulario.php, front_php\vistas\inicio.php, front_php\vistas\lista.php, front_php\vistas\no_encontrada.php, front_php\vistas\plantilla.php, pruebas_humo\humo_front.py
```
...
tú creas mejor:
  ...
  · docs/spec_kit/versiones/v1_programa/2_spec.md    qué hay que construir
  · docs/spec_kit/versiones/v1_programa/6_contracts.md   cada ruta, exacta
...
3. La tabla es `programa`, con llave `id` y estos campos:
   id (entero), nombre (texto), tipo (texto), nivel (texto), fecha_creacion (texto), numero_cohortes (texto), cant_graduados (texto), fecha_actualizacion (texto), ciudad (texto), facultad (entero), fecha_cierre (texto, opcional)
...
6. La base YA VIENE DADA en db/init.sql. No la modifiques ni escribas SQL de
   creación de tablas. La tabla `programa` arranca con 191
   filas sembradas.
...
9. LA VERSIÓN INCLUYE SU PANTALLA, y es la mitad del trabajo, no un añadido.
   ...
   publicando el puerto 8112:

   · una pantalla por recurso, con DIRECCIÓN PROPIA (/programas),
     nunca una ruta con el nombre de la tabla como parámetro;
   · una FUNCIÓN POR OPERACIÓN —listar_gestion, crear_gestion…—,
   ...
   d) Y NO compartas código entre la API y el front. Las dos están en PHP y
      ...
      require_once __DIR__ . '/../api_gestion/modelos/Programa.php'
...
| **`activo` en el modelo o en la lista blanca** | Es una columna de la tabla, parece un campo más | ¿`Programa::toArray()` lo devuelve? No debe |
| **Un cliente genérico en el front** | Es más corto, y con una sola tabla ni se nota | ¿Las funciones se llaman `listar_gestion` o `listar($recurso)`? |
| **`require` del modelo de la API en el front** | Están ahí al lado y ahorra escribirlos | ¿Hay algún `require` que apunte a `../api_gestion/`? |

## 4. Y lo que la IA no va a hacer sola

Comparar con el gemelo. Este mismo módulo está en
`proyecto_paradigmas_gestion_profesoral1`, en Python y FastAPI. Levante los dos,
