...
```
GET /api/programa HTTP/1.1          ← un GET: verbo + ruta, sin body
```

```
POST /api/programa HTTP/1.1         ← un POST: verbo + ruta...
...
| **La ruta** | `$ruta = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);` | `$ruta` vale `"/api/programa"` |
...
```php
if ($ruta === '/api/programa') {          // ¿pidieron la colección?
    if ($metodo === 'GET') {
...
`POST /api/programa` con `{"codigo":"PR009","nombre":"Webcam","stock":10,"valorunitario":350000}`:

```
1. index.php        captura  $metodo="POST", $ruta="/api/programa", $body=[...]
2. index.php        compara  y llama $controlador->crear($body)
3. ControladorPrograma::crear
...
4. ServicioPrograma::crear
      construye el objeto del modelo (new Programa(...))
      y se lo pasa al repositorio (por la interfaz)
5. RepositorioProgramaMariaDB::crear
      INSERT INTO programa (...) VALUES (:codigo, :nombre, ...)  ← PDO preparado
...
```mermaid
flowchart TD
    A["1. php -S / php-fpm recibe<br/>el texto HTTP"] --> B["2. index.php encuentra<br/>POST /api/programa<br/>en la tabla de rutas"]
...
```
1. index.php        captura  $metodo="GET", $ruta="/api/programa/PR001"
2. index.php        recorta  el código con substr → "PR001"
                    y llama  $controlador->obtener("PR001")
3. ControladorPrograma::obtener   try { responder(200, servicio->obtener(...)) }
4. ServicioPrograma::obtener      pide al repositorio; si llega null → lanza
                                  NoEncontradoExcepcion (el catch la vuelve 404)
5. RepositorioProgramaMariaDB     SELECT ... WHERE codigo = :codigo
6. La fila vuelve convertida en objeto Programa y sale como JSON
```
...
```mermaid
flowchart LR
    A["GET /api/<br/>programa/PR001"] --> B["controlador"] --> S["servicio"] --> R["repositorio:<br/>SELECT ... WHERE<br/>codigo = :codigo"]
    R --> E{"¿hay fila?"}
    E -->|"sí"| OK["200: el programa<br/>en JSON"]
...
Invoke-RestMethod "http://localhost:8113/api/programa"
Invoke-RestMethod "http://localhost:8113/api/programa/PR001"

# POST — crear
Invoke-RestMethod -Method Post -Uri "http://localhost:8113/api/programa" -ContentType "application/json" -Body '{"codigo":"PR009","nombre":"Webcam","stock":10,"valorunitario":350000}'

# PUT con body incompleto → error 422 (PUT exige TODO)
Invoke-RestMethod -Method Put -Uri "http://localhost:8113/api/programa/PR009" -ContentType "application/json" -Body '{"stock":25}'

# PATCH con el MISMO body → 200 (PATCH es parcial)
Invoke-RestMethod -Method Patch -Uri "http://localhost:8113/api/programa/PR009" -ContentType "application/json" -Body '{"stock":25}'

# DELETE — limpiar
Invoke-RestMethod -Method Delete -Uri "http://localhost:8113/api/programa/PR009"
```
