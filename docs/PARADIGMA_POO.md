...
Problema: saber cuántas fichas activas hay por ciudad. Mírelo tres veces:
...
SELECT ciudad, COUNT(*) FROM programa
WHERE activo = TRUE GROUP BY ciudad;
```

```
// P.O.O.: objetos que colaboran — cada uno con SU responsabilidad
$servicio->contarPorCiudad();   // el servicio le PIDE al repositorio;
...
```mermaid
flowchart TB
    subgraph PROY["El proyecto api_gestion — multiparadigma a propósito"]
...
### 2.1 Abstracción
Quedarse con lo esencial y esconder el detalle. `IRepositorioPrograma` es una
abstracción: define QUÉ se puede hacer con programas (obtener, crear,
actualizar, eliminar) sin decir CÓMO ni DÓNDE se guardan.

### 2.2 Encapsulamiento
Cada objeto guarda su estado y expone solo operaciones. En la v1,
`RepositorioProgramaMariaDB` encapsula la conexión PDO y el SQL: sus
...
`ServicioPrograma` no HEREDA de un repositorio, RECIBE un repositorio por
...
`implements IRepositorioPrograma` puede ocupar el lugar de otra — el MariaDB
...
classDiagram
    ...
    class RepositorioProgramaMariaDB {
    ...
    class ServicioPrograma {
        -repositorio: IRepositorioPrograma
        +reglas de negocio (límite mayor que 0, ...)
    }
    RepositorioProgramaMariaDB ..|> IRepositorioPrograma : POLIMORFISMO
    RepositorioFalsoEnMemoria ..|> IRepositorioPrograma : POLIMORFISMO
    ServicioPrograma o-- IRepositorioPrograma : COMPOSICIÓN (recibe, no hereda)
    note for IRepositorioPrograma "ABSTRACCIÓN: declara QUÉ — ni una línea de CÓMO"
    note for RepositorioProgramaMariaDB "ENCAPSULAMIENTO: la conexión y el SQL no salen de aquí"
```
...
Cuando `ServicioPrograma` manda el mensaje `crear(datos)`, NO sabe (ni le
...
classDiagram
    direction LR
    class ServicioMal["ServicioPrograma ❌"]
    class ServicioBien["ServicioPrograma ✅"]
    ServicioMal --|> RepositorioProgramaMariaDB : hereda del CONCRETO — quedó casado con MariaDB
    ServicioBien o-- IRepositorioPrograma : compone la ABSTRACCIÓN — cualquier motor entra
```
...
sequenceDiagram
    ...
    participant C as ControladorPrograma
    participant S as ServicioPrograma
    participant R as IRepositorioPrograma (interface)
    participant BD as MariaDB
    Cli->>C: POST /api/programa (JSON)
...
- **El modelo es el dato como objeto**: la clase `Programa` con sus 4
  propiedades tipadas — un `ciudad` que SIEMPRE es entero.
...
1. **El dominio se modela solo:** programa, registro, cliente… son objetos
...
```
Programa (el modelo)         ← la clase entidad: el dato con tipos
ControladorPrograma          ← objeto HTTP; valida el body y compone un IServicioPrograma
ServicioPrograma             ← objeto de NEGOCIO; compone un IRepositorioPrograma
IRepositorioPrograma         ← contrato (interface): abstracción pura
RepositorioProgramaMariaDB   ← implementación concreta (encapsula PDO y SQL)
RepositorioFalsoEnMemoria    ← otra implementación (¡polimorfismo!) para probar sin BD
```

El mismo `ServicioPrograma` funciona con ambos repositorios sin cambiar una
línea — eso es el paradigma haciendo su trabajo. En la v3, un tercer objeto
(`RepositorioProgramaPostgreSQL`) entrará por la misma puerta.
...
6. En este repositorio: las interfaces y capas de la
   [v1](spec_kit/versiones/v1_programa/3_plan.md).
