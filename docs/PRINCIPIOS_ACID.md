...
módulo y carga las 191 filas de la tabla `programa`. Si el motor fallara a la
...
llave primaria de `programa` impide dos fichas con el mismo `id` —
...
programa al tiempo. Sin aislamiento, las dos leen «está activo» y las dos
...
Un módulo académico está del lado izquierdo: un programa no puede existir a
...
[7_quickstart.md](spec_kit/versiones/v1_programa/7_quickstart.md), que
